...
import { INITIAL_PATTERNS, WEEKLY_DATA_MOCK } from '../constants';
...
export const EmotionDashboard: React.FC = () => {
  ...
  const [analytics, setAnalytics] = useState({
    weeklyData: WEEKLY_DATA_MOCK,
    balanceMetrics: [
      { subject: 'Stress', A: 65, fullMark: 100 },
      { subject: 'Anxiety', A: 45, fullMark: 100 },
      { subject: 'Mood Stability', A: 70, fullMark: 100 },
      { subject: 'Energy', A: 55, fullMark: 100 },
      { subject: 'Emotional Load', A: 40, fullMark: 100 },
    ],
  ...
  useEffect(() => {
    if (user?.id) {
      try {
        const userAnalytics = journalStore.getMoodAnalytics(user.id);
        const balanceMetrics = [
          { subject: 'Stress', A: userAnalytics.stressLevel, fullMark: 100 },
          { subject: 'Anxiety', A: userAnalytics.anxietyLevel, fullMark: 100 },
          { subject: 'Mood Stability', A: userAnalytics.averageMood, fullMark: 100 },
          { subject: 'Energy', A: userAnalytics.energyLevel, fullMark: 100 },
          { subject: 'Emotional Load', A: Math.round((100 - userAnalytics.averageMood) * 0.9), fullMark: 100 },
        ];

        setAnalytics({
          weeklyData: userAnalytics.weeklyData,
          balanceMetrics,
          patterns: INITIAL_PATTERNS.map((pattern, idx) => ({
            ...pattern,
            score: Math.round(Math.random() * 40 + 30 + (userAnalytics.averageMood - 50) * 0.5)
          }))
        });
      } catch (error) {
